...
class Ground:
    def __init__(self, name: str, type_: str = "random") -> None:
        ...
        self.TD_MAX: float = None
        self.TD_MIN: float = None
        self.TU_MAX: float = None
        self.TU_MIN: float = None
        ...
        self.points_index: np.ndarray = self.generate_point_index()

        # useful attributes for cases
        self.sigma: int = int(np.ceil(self.__dict__["lk_under"] / self.ds))
        self.e6g: np.ndarray = np.hstack(
            (self.points_index[0, 1], self.points_index[:, 1], self.points_index[-1, 1]))
        self.iy_min: int = self.points_index[:, 1].min()
        self.iy_max: int = self.points_index[:, 1].max()
        self.potential_vpi: np.ndarray = self.get_potential_vpi_array()
        self.ek_lim = self.get_ek_lim_array()
        self.curve_resist = self.get_curve_resist_array()
        self.num_s = self.e6g.size - 2
        return
    ...
    def get_discrete_points(self) -> np.ndarray:
        dis_points = self.points_index.copy()
        dis_points[:, 0] = dis_points[:, 0] * self.ds + self.bottom_left_corner_coordinates[0]
        dis_points[:, 1] = dis_points[:, 1] * self.de + self.bottom_left_corner_coordinates[1]
        return dis_points
...
def main():
    gd = Ground(name="gd1", type_="random")
    # print(gd.get_stair_plot_data())
