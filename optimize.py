...
class TC(OptimizationModel):
    def __init__(self, train: Train, track: Track, is_ee: bool = True, VI_on: bool = True,
                 warm_start_model: Union[dict, "OptimizationModel"] = None):
        name = "eetc" if is_ee else "sotc"
        VI_on = VI_on if is_ee else False
        if VI_on:
            name += "_tcVI"
        ...
        self.set_warm_start(warm_start_model)
        pass
...
class VAOTC(OptimizationModel):
    def __init__(self, ground: Ground, train: Train, name: str):
        super().__init__(name, ground, train)


def main():
    vao = VAO(ground=Ground("gd4"), VI_on=False, LC_ON=False)
    vao.optimize()
    track = vao.get_track()
    sotc = TC(train=Train("CRH380AL"), track=track, is_ee=False)
    sotc.optimize()
    eetc1 = TC(train=Train("CRH380AL"), track=track, is_ee=True, VI_on=False, warm_start_model=sotc)
    eetc1.optimize()
    eetc2 = TC(train=Train("CRH380AL"), track=track, is_ee=True, VI_on=True)
    eetc2.optimize()
    pass
